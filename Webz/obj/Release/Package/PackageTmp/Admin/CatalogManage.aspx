    ...
    <form class="layui-form" action="" runat="server">
        <div>
            <div style="min-height: 600px;">
                <div id="rightcontent">
                    <div>
                        <div style="font-size: 12px;margin-left:12px;">当前位置：教材目录管理</div>
                        <div id="tiaojian">
                        ...
                        <div style="height: 20px;margin-top: 6px;">
                            <button type="button" id="btnDelete" class="btnDelete layui-btn layui-btn-normal" style="margin-right: 4%; float: right; width: 72px;">删除</button>
                            <button type="button" id="btnAdds" class="btnAdds layui-btn layui-btn-normal" style="margin-right: 1%; float: right; width: 72px;margin-left: 0px;">导入</button>
                            <button type="button" id="btnAdd" class="btnAdd layui-btn layui-btn-normal" style="margin-right: 1%; float: right; width: 72px;">新增</button>
                        </div>
                    </div>
                ...
                <script>
                    ...
                    $(function () {
                        ...
                        function SelectList(data) {
                            $(".cheks").attr("checked", false);
                            $(".cheks").css("background-image", "url(Img/打勾20.png)");
                          ...
                          , count: count, //数据总数，从服务端得到
                            ...
                            for (var i = 0; i < data.list.length; i++) {
                                ...
                                $("#huadong table").append("<tr><td>" + data.list[i].CaID + "</td>" +
                                    ...
                                    "<td class='tdPassWord'><a href='#' onclick='Viewdialog2(" + data.list[i].CaID + ")' class='layui-btn layui-btn-sm But_Video'>修改</a></td>" +
                                    "<td>" +
                                        "<input type='hidden' name='chekname' value='0' />" +
                                        "<span class='chek'></span>" +
                                        "<input type='hidden' value='' />" +
                                        "<input type='checkbox' lay-filter='BDing' class='chek' name='chek' id='' value='' CaID=" + data.list[i].CaID + " />" +
                       ...
                       //新增按钮
                        $(".btnAdd").click(function () {
                            window.location.replace("BooksCataLog.aspx?type=add");
                        });
                        ...
                        $(".btnDelete").click(function () {
                            var checked = $("input[class=chek]:checked");
                            if (checked.length > 0) {
                                var bol = confirm("确定要删除吗？");
                                if (bol == true) {
                                    var arraystr = new Array();
                                    for (var i = 0; i < checked.length; i++) {
                                        arraystr[i] = $(checked[i]).attr("CaID");
                                    }
                                    $.post("AjaxHelper/BooksCataLog.ashx", "action=batchdelete&array=" + arraystr,
                                        function (data) {
                                            if (data > 0) {
                                                alert("删除成功！");
                                            } else {
...
<script>
    ...
    function Viewdialog2(data) {
        //alert("功能在开发中");
        window.location.replace("BooksCataLog.aspx?type=update&CaID=" + data);
    }
